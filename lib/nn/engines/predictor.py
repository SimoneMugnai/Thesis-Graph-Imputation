...
class MissingDataPredictor(Predictor):
    ...
    def __init__(self,
                 ...
                 log_scores_options: Optional[Mapping] = None,
                 *,
        ...
        self.log_scores_options = log_scores_options
    ...
    def predict_step(self, batch, batch_idx, dataloader_idx=None):
        ...
        # Make predictions
        y_hat, _, = self.predict_batch(batch,
                                       preprocess=False,
                                       postprocess=True)
    ...
    def compute_imputation_loss(self, x_hat, batch, scale_target, step: str):
        assert step == 'train'

        mask = batch.original_mask
        if isinstance(self.model, SPINHierarchicalPredictionModel):
            assert self.whiten_prob > 0, \
                ("Cannot use imputation loss on spin-like models without "
                 "injecting missing values (whiten_prob > 0).")
            mask = batch.original_mask ^ batch.input_mask  # XOR
    ...
    def compute_prediction_loss(self, y_hat, batch, scale_target, step: str):
    ...
    def training_step(self, batch, batch_idx):
        ...
        y_hat, x_hat = self.predict_batch(batch,
                                          preprocess=False,
                                          postprocess=not self.scale_target)

        loss = self.compute_prediction_loss(y_hat,
                                            batch,
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # if self.imputation_loss_fn is not None and x_hat is not None:
        #    loss += self.imputation_loss_weight * self.compute_imputation_loss(
        #        x_hat, batch, scale_target=self.scale_target, step='val'
        #    )
        return loss
